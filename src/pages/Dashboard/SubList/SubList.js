...
const SubList = ({ subscriptions, setActivePlan, activePlan }) => {
    const isActivePlan = activePlan!==null
            ...
            {subscriptions.map((subscriptionPlanData, index) => {
                ...
                return (
                    <button key={index} id={`sub-${index}`} className="sublist__sub-btncontainer" onClick={() => setActivePlan(index)}>
                        <div className={`sublist__sub ${isActivePlan && 'minimized'}`} style={ index === activePlan ? {backgroundColor: `${subscriptionData.colourCode}`} : {}}>
